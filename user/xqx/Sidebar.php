...
.sidebar-logo-area {
    ...
    /* 主分类基础项 以及 关于独立链接的基础公用样式 */
    .album-category, .home-link, .sidebar-link-direct {
        display: flex;
        align-items: center;
    ...
    .album-category:hover, .home-link:hover, .sidebar-link-direct:hover {
        color: #fff !important;
    }
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        // 2. 为其他各主分类动态创建独立抽屉（已跳过不需要动态创建的 3d）
        albumCategories.forEach(category => {
            const categoryName = category.dataset.category;
            if (categoryName === 'all' || categoryName === '3d') return;
